...
public:
    tree_node_();
    explicit tree_node_(const T &);
    explicit tree_node_(T &&);
...
{
public:
    explicit navigation_error(const std::string &s) : std::logic_error(s)
    {
        //			assert(1==0);
...
public:
    ...
    explicit tree(const T &); // constructor setting given element as head
...
#    endif
    ...
    public:
        pre_order_iterator();
        explicit pre_order_iterator(tree_node *);
        explicit pre_order_iterator(const iterator_base &);
        explicit pre_order_iterator(const sibling_iterator &);
    ...
    public:
        post_order_iterator();
        explicit post_order_iterator(tree_node *);
        explicit post_order_iterator(const iterator_base &);
        explicit post_order_iterator(const sibling_iterator &);
    ...
    public:
        ...
        explicit breadth_first_queued_iterator(tree_node *);
        explicit breadth_first_queued_iterator(const iterator_base &);
    ...
    public:
        ...
        explicit fixed_depth_iterator(tree_node *);
        explicit fixed_depth_iterator(const iterator_base &);
        explicit fixed_depth_iterator(const sibling_iterator &);
    ...
    public:
        sibling_iterator();
        explicit sibling_iterator(tree_node *);
        sibling_iterator(const sibling_iterator &);
        explicit sibling_iterator(const iterator_base &);
    ...
    public:
        leaf_iterator();
        explicit leaf_iterator(tree_node *, tree_node *top = 0);
        explicit leaf_iterator(const sibling_iterator &);
        explicit leaf_iterator(const iterator_base &);
    ...
    /// position. Does not change the subtree itself (use move_in or move_in_below for that).
    template<typename iter>
    ...
    /// present.
    void merge(sibling_iterator, sibling_iterator, sibling_iterator, const sibling_iterator &,
    ...
    /// Compute the depth distance between two nodes, counting all levels for which predicate
    /// returns true.
    template<class Predicate>
    ...
    /// Count the number of siblings (left and right) of node at iterator. Total nodes at this level
    /// is +1.
    ...
    /// Determine whether the iterator is one of the 'head' nodes at the top level, i.e. has no
    /// parent.
...
private:
    ...
    {
    public:
        explicit compare_nodes(StrictWeakOrdering comp) : comp_(comp) { }
...
{
    head = std::allocator_traits<decltype(alloc_)>::allocate(alloc_, 1, nullptr);
    feet = std::allocator_traits<decltype(alloc_)>::allocate(alloc_, 1, nullptr);
...
template<class T, class tree_node_allocator>
tree<T, tree_node_allocator> &
tree<T, tree_node_allocator>::operator=(const tree<T, tree_node_allocator> &other)
{
    if (this != &other)
        copy_(other);
    return *this;
...
tree<T, tree_node_allocator>::begin_fixed(const iterator_base &pos, unsigned int dp,
...
tree<T, tree_node_allocator>::begin(const iterator_base &pos)
{
    ...
    return tree<T, tree_node_allocator>::sibling_iterator(pos.node->first_child);
}
...
tree<T, tree_node_allocator>::path_from_iterator(const iterator_base &iter,
...
tree<T, tree_node_allocator>::iterator_from_path(const path_t &path, const iterator_base &top) const
{
    ...
    for (size_t step = 0; step < path.size(); ++step) {
        ...
        if (walk == 0)
            throw std::range_error("tree::iterator_from_path: no more nodes at step "
                                   + std::to_string(step));

        for (int i = 0; i < path[step]; ++i) {
            walk = walk->next_sibling;
            if (walk == 0)
                throw std::range_error("tree::iterator_from_path: out of siblings at step "
                                       + std::to_string(step));
        }
    }
    it.node = walk;
...
iter tree<T, tree_node_allocator>::append_child(iter position, T &&x)
{
    ...
    tree_node *tmp = std::allocator_traits<decltype(alloc_)>::allocate(alloc_, 1, nullptr);
...
iter tree<T, tree_node_allocator>::replace(iter position, const iterator_base &from)
{
    ...
    pre_order_iterator toit = pre_order_iterator(tmp);
    // copy all children
    do {
...
template<class T, class tree_node_allocator>
template<class Predicate>
int tree<T, tree_node_allocator>::distance(const iterator_base &top, const iterator_base &bottom,
                                           Predicate p)
{
...
template<class T, class tree_node_allocator>
typename tree<T, tree_node_allocator>::iterator
tree<T, tree_node_allocator>::lowest_common_ancestor(const iterator_base &one,
